...
def draw_boxes(img, bbox, identities=None, categories=None, names=None,offset=(0, 0)):
    ...
    for i, box in enumerate(bbox):
        ...
        color = [0,255,0]
        #label = f"Num:{id} and its{cat}"
        label = str(cat)
        (w, h), _ = cv2.getTextSize(cat, cv2.LINE_AA, 1, 2)
        cv2.rectangle(img, (x1, y1), (x2, y2), color, 1)
        cv2.rectangle(img, (x1, y1 - 20), (x1 + w, y1), color, -1)
        cv2.putText(img, cat, (x1, y1 - 5),cv2.LINE_AA, 1, [255, 255, 255], 2)
        #cv2.putText(img,f"Total number : {str(max(count_list))}", (20,25), cv2.LINE_AA,1, color ,2)
...
@torch.no_grad()
def detect(weights=ROOT / 'garbage.pt',
        source=ROOT / '', 
